...
describe('RuleChecker', () => {
  describe('Is valid child', () => {
    ...
    it('Invalid children evaluate to false (4)', () => {
      ...
      assert.isFalse(ruleChecker.isValidChild(latteHotKey, anotherSprinkleKey));
    });
  ...
  describe('getIncrementalMutualExclusionPredicate()', () => {
    it('general', () => {
      const f = ruleChecker.getIncrementalMutualExclusionPredicate(latteHotKey);
  ...
  describe('Within quantity threshold', () => {
    it('Valid within bounds', () => {
      assert.isTrue(ruleChecker.isValidQuantity(smallIced, drizzleKey, 2));
      assert.isTrue(ruleChecker.isValidQuantity(smallIced, drizzleKey, 5));
      assert.isTrue(ruleChecker.isValidQuantity(smallIced, drizzleKey, 9));

      assert.isTrue(ruleChecker.isValidQuantity(mediumIced, drizzleKey, 2));
      assert.isTrue(ruleChecker.isValidQuantity(mediumIced, drizzleKey, 7));
      assert.isTrue(ruleChecker.isValidQuantity(mediumIced, drizzleKey, 14));

      assert.isTrue(ruleChecker.isValidQuantity(largeIced, drizzleKey, 2));
      assert.isTrue(ruleChecker.isValidQuantity(largeIced, drizzleKey, 8));
      assert.isTrue(ruleChecker.isValidQuantity(largeIced, drizzleKey, 19));
    });

    it('Valid on edge of bounds', () => {
      assert.isTrue(ruleChecker.isValidQuantity(smallIced, drizzleKey, 0));
      assert.isTrue(ruleChecker.isValidQuantity(smallIced, drizzleKey, 1));
      assert.isTrue(ruleChecker.isValidQuantity(smallIced, drizzleKey, 10));

      assert.isTrue(ruleChecker.isValidQuantity(mediumIced, drizzleKey, 0));
      assert.isTrue(ruleChecker.isValidQuantity(mediumIced, drizzleKey, 1));
      assert.isTrue(ruleChecker.isValidQuantity(mediumIced, drizzleKey, 15));

      assert.isTrue(ruleChecker.isValidQuantity(largeIced, drizzleKey, 0));
      assert.isTrue(ruleChecker.isValidQuantity(largeIced, drizzleKey, 1));
      assert.isTrue(ruleChecker.isValidQuantity(largeIced, drizzleKey, 20));
    });

    it('Invalid outside of bounds', () => {
      assert.isFalse(ruleChecker.isValidQuantity(smallIced, drizzleKey, -3));
      assert.isFalse(ruleChecker.isValidQuantity(smallIced, drizzleKey, -1));
      assert.isFalse(ruleChecker.isValidQuantity(smallIced, drizzleKey, 11));
      assert.isFalse(ruleChecker.isValidQuantity(smallIced, drizzleKey, 47));

      assert.isFalse(ruleChecker.isValidQuantity(mediumIced, drizzleKey, -7));
      assert.isFalse(ruleChecker.isValidQuantity(mediumIced, drizzleKey, -1));
      assert.isFalse(ruleChecker.isValidQuantity(mediumIced, drizzleKey, 16));
      assert.isFalse(ruleChecker.isValidQuantity(mediumIced, drizzleKey, 56));

      assert.isFalse(ruleChecker.isValidQuantity(largeIced, drizzleKey, -5));
      assert.isFalse(ruleChecker.isValidQuantity(largeIced, drizzleKey, -1));
      assert.isFalse(ruleChecker.isValidQuantity(largeIced, drizzleKey, 21));
      assert.isFalse(ruleChecker.isValidQuantity(largeIced, drizzleKey, 84));
    });

    it('Invalid items return false', () => {
      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, drizzleKey, 2));
      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, drizzleKey, 5));
      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, drizzleKey, 9));

      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, drizzleKey, 0));
      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, drizzleKey, 1));
      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, drizzleKey, 10));

      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, drizzleKey, -3));
      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, drizzleKey, -1));
      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, drizzleKey, 11));
      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, drizzleKey, 47));
    });

    it('Wildcard partialKeys specify rules for all downstream keys', () => {
      assert.isTrue(ruleChecker.isValidQuantity(latteHotKey, twoMilkKey, 1));
      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, twoMilkKey, 0));
      assert.isFalse(ruleChecker.isValidQuantity(latteHotKey, twoMilkKey, 2));
    });

    it('Wildcards work on fully qualified keys', () => {
      assert.isTrue(ruleChecker.isValidQuantity(largeIced, whippedCreamKey, 1));
      assert.isFalse(
