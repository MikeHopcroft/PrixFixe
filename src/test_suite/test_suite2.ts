...
import { TestLineItem, TestCounts, TestOrder, XmlNode, YamlTestCase, TestSteps } from './interfaces';

const debug = Debug('prix-fixe:TestSuite.fromYamlString');


export type SpeechToTextSimulator = (text: string) => TestSteps;
            ...
            for (const [i, step] of this.steps.entries()) {
                ...
                if (
                    ...
                    succeeded = isomorphic
                        ? ordersAreEqualCanonical(observed, expected.cart)
                        : ordersAreEqual(observed, expected.cart);
                }
            }
        } catch (e) {
            succeeded = false;
