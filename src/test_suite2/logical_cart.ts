...
export function logicalCartFromCart(cart: Cart, catalog: ICatalog): LogicalCart {
    return {
        items: cart.items.map(item => logicalItemFromItemInstance(item, catalog)),
    };
}

function logicalItemFromItemInstance(item: ItemInstance, catalog: ICatalog): LogicalItem {
    ...
    return {
        ...
        children: item.children.map(
            child => logicalItemFromItemInstance(child, catalog)
        ),
    };
}

export function cartFromlogicalCart(cart: LogicalCart, catalog: ICatalog): Cart {
    return {
...
function itemInstanceFromLogicalItem(item: LogicalItem, catalog: ICatalog): ItemInstance {
